...
class AuthController {
    constructor() {
        ...
        this.register = async (req, res) => {
            ...
            if (!(0, validation_1.useCheckOnEmail)(email)) {
                return res.status(400).json({
                    message: "Incorrect email, Please write correct email",
                });
            }
            if (!(0, validation_1.useCheckOnPassword)(password))
                return res.status(400).json({
                    message: "Incorrect Password, Password should be greater than 6 and shouldn't include special character only characters, numbers and underscore",
                });
            (0, database_1.default)()
                .then(async (hasConnected) => {
                if (!hasConnected)
                    throw new Error("Sorry!, Server couldn't handle your request. Try some time later");
                const hasUser = await User_1.default.findOne({ email: email.toLowerCase() });
                if (hasUser) {
                    ...
                    try {
                        ...
                        console.error("Register: sendOTPEmail error (existing user):", otpErr?.message || otpErr);
                    }
                ...
                try {
                    ...
                    try {
                        ...
                        console.error("Register: sendOTPEmail error (new user):", otpErr?.message || otpErr);
                    }
                    if (sent) {
                        return res.status(201).json({
                            message: "Your account has been created. Please check your email for the OTP.",
                        });
                    }
        ...
        this.sendVerificationEmail = async (user) => {
            try {
                ...
                await (0, emailService_1.default)({
                    ...
                    html: (0, emailTemplates_1.getVerificationEmailTemplate)(user.firstName, link),
                });
                return true;
            }
            catch (err) {
        ...
        this.sendOTPEmail = async (user) => {
            try {
                ...
                await (0, emailService_1.default)({
                    ...
                    html: (0, emailTemplates_1.getOTPEmailTemplate)(user.firstName, otp),
                });
                return true;
            }
            catch (err) {
        ...
        this.forgotPasswordOTP = async (req, res) => {
            try {
                ...
                if (!(0, validation_1.useCheckOnEmail)(emailRaw)) {
                    return res.status(400).json({
                        success: false,
                ...
                try {
                    ...
                    console.error("Forgot password: sendOTPEmail error:", otpErr?.message || otpErr);
                }
                if (!sent) {
                    return res.status(500).json({
                        success: false,
        ...
        this.verifyOtp = async (req, res) => {
            try {
                ...
                const isForgotPassword = forgotPassword === true || forgotPassword === "true";
                if (!email || !otpTrim) {
                    return res.status(400).json({
                        success: false,
                ...
                (0, emailService_1.default)({
                    ...
                    html: (0, emailTemplates_1.getWelcomeEmailTemplate)(user.firstName),
        ...
        this.resendOtp = async (req, res) => {
            try {
                ...
                const user = await User_1.default.findOne({ email: emailTrim.toLowerCase() });
                if (!user) {
                    return res.status(404).json({
                        success: false,
                        message: "User not found",
        ...
        this.resetPassword = async (req, res) => {
            try {
                ...
                if (!(0, validation_1.useCheckOnPassword)(newPassword)) {
                    return res.status(400).json({
                        success: false,
                        message: "Password should be greater than 6 and only include characters, numbers and underscore",
                    });
                }
                const email = emailRaw.toLowerCase();
                const user = await User_1.default.findOne({ email });
                ...
                if (!user.passwordResetToken ||
                    ...
                    return res.status(400).json({
                        success: false,
                        message: "OTP verification required. Please complete OTP verification from forgot password again.",
                    });
                }
            ...
            try {
                ...
                if (!user.isEmailVerified) {
                    ...
                    try {
                        ...
                        console.error("Login: sendOTPEmail error:", otpError?.message || otpError);
                    }
        ...
        this.updateProfile = async (req, res) => {
            try {
                ...
                const updatedUser = await User_1.default.findByIdAndUpdate(user._id, { firstName, lastName }, { new: true, runValidators: true }).select("-password");
                res.json({
                    success: true,
        ...
        this.onboardUser = async (req, res) => {
            try {
                const userId = req.user._id;
                const { firstName, lastName, gender, dob, region, bio, subscriptionTier, } = req.body;
        ...
        this.updateDetailedProfile = async (req, res) => {
            try {
                const userId = req.user._id;
                const { firstName, lastName, gender, dob, region, bio, avatar } = req.body;
                const updatedUser = await User_1.default.findByIdAndUpdate(userId, { firstName, lastName, gender, dob, region, bio, avatar }, { new: true, runValidators: true }).select("-password");
                res.json({
                    success: true,
        ...
        this.updateSubscription = async (req, res) => {
            try {
                ...
                const updatedUser = await User_1.default.findByIdAndUpdate(userId, { subscriptionTier: tier }, { new: true }).select("-password");
                res.json({
                    success: true,
        ...
        this.getSystemOverview = async (req, res) => {
            ...
            try {
                ...
                const memoryCount = await MemoryChunk_1.default.countDocuments({ userId });
                const avatarCount = await Avatar_1.default.countDocuments({ userId });
                const activeAvatar = await Avatar_1.default.findOne({ userId }).sort({
                    updatedAt: -1,
                });
    ...
    async resendVerification(req, res) {
        try {
            ...
            await (0, emailService_1.default)({
                ...
                html: (0, emailTemplates_1.getVerificationEmailTemplate)(user.firstName, verificationLink),
            });
            res.json({
                success: true,
